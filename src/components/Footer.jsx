...
const Footer = () => {
  return (
    <div className='px-4 sm:px-12 lg:px-24 xl:px-40  font-medium text-md pt-10 pb-5  bg-gray-900 max-sm:mt-15 flex max-sm:flex-col text-white gap-80 max-sm:gap-20 md:gap-40 lg:gap-40 xl:gap-80 sm:gap-20' >
      <div >
...
<div className='flex-col flex text-center '>
    ...
    <Link to="/" onClick={()=>{ setSidebar(false), window.scrollTo({top:0, behavior:"smooth"})}} className='  hover:text-primary rounded-full text-base font-medium cursor-pointer  max-sm:text-white' href='#'>Home</Link>
    <Link to="/#About-us" onClick={()=> setSidebar(false)} className='  hover:text-primary rounded-full text-base font-medium cursor-pointer' href='#About-us'>About us</Link>
    <Link to="/#Structure"  onClick={()=> setSidebar(false)} className='hover:text-primary rounded-full text-base font-medium cursor-pointer' href='#Structure'>Structure</Link>
    <Link to='/history' onClick={()=> {setSidebar(false), window.scrollTo({top:0, behavior:'smooth'})} } className='hover:text-primary  rounded-full text-base font-medium cursor-pointer ' href='#History'>History</Link>
    <Link to="/#Contact-us" onClick={()=> setSidebar(false)} className='hover:text-primary  rounded-full text-base font-medium cursor-pointer ' href='#Contact-us'>Contact us</Link>
...
<div className="pt-5">
  ...
  <div className='flex gap-3 mt-5 md:justify-center max-sm:justify-center'>
    <a  href="https://www.facebook.com/epsf.benisuef?__cft__[0]=AZYTvLbGTPZ3cabK9p1-YOOqAOLFtqTZlAt0ez1DOdNnBJPa8AYprKnMu0ND260xC8BH4wS5AGsWkWsz_D1ip5KES2L0U2MSi0nbFhRIPxIhrsU_34YnKYGLZsfA9NEDFk80_CH0_dN-znPcEc3p5ZMtjaWMNxiSQPa14HqwfWZf5A&__tn__=-]K-R"><FaFacebookF  /></a>
